...
def init_argparse() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        usage="%(prog)s [OPTION] ...",
        description="Use tcpmon to measure TCP throughput as a function of message spcacing."
    )
    parser.add_argument(
        "-a", action='store', 
        default=' ',
...
# DTNlon - DTNpar
msg_spacings = ["0",  "15", "20", "25", "30", "40", "50", "60", "75", "100", "300" ]
...
def main() -> None:
    ...
    # loop over the message spacings 
    index = 0
    for msg_spacing in msg_spacings:
        ...
        # make the command
        cmd = cmd_test+" -d "+args.d+" -p "+str(args.p) +loops+run_time+affinity+interval+ " -S "+str(args.S)+ " -w "+msg_spacing
        print(cmd)
